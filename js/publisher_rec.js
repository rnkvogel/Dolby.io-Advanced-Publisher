 ...
 */
  const applyAllConstraints = async () => {
    ...
    function updateRecordButtonUI() {
        ...
        const pub = getPublisher && getPublisher();
        const recordingAvailable = !!(pub && typeof pub.recordingAvailable === 'boolean'
            ? pub.recordingAvailable
            : canRecordToken);

        // Reset classes
        btn.classList.remove('d-none', 'hidden', 'record-armed', 'record-ready', 'record-live');

        if (!window.isBroadcasting) {
            // Pre-publish: show only if allowed
            if (recordingAvailable || canRecordToken) {
                btn.disabled = true;
                btn.textContent = startWithRecord ? 'Recording' : 'Record';
        ...
        if (isRecording) {
            ...
            btn.classList.add('record-live');
        } else if (recordingAvailable) {
            btn.disabled = false;
            btn.textContent = 'Start Rec';
            btn.classList.add('record-ready');
        } else {
            btn.classList.add('d-none');
            btn.disabled = true;
        }
